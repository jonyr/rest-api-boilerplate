...
class ValidatorEngine(object):
    ...
    The library uses function decorator pattern, which means the incomming request data is validated before the request
    hit the flask route function
    ...
    Built-in Validation Rules
        >>> json: using the json rule, the validator expects a JSON object from the client. It validates fields
            in JSON data.
    ...
    Built-in Validation Rules Args
        ...
        >>> list: This check that the input under validation is a list. You can also set your validation to make sure
            the list is of a specific length
        ...
        >>> date_after_or_equal: Checks that the date matches the format and is after or equals to the
            date value specified
        ...
        def wrapper(func):
            ...
            def inner_wrapper(*args, **kwargs):
                try:
                    ...
                    raise Exception(
                        f"""AttributeError {validation_type} passed, expecting json or form_data or
                        query_string or headers"""
                    ) from AttributeError
    ...
    def check_values(self, data, validation_rules):
        for field, rules in validation_rules.items():
            for rule in rules:
                ...
                try:
                    validation_result = getattr(Validators, validator_name)(data.get(field, None), *validator_args)
                except KeyError:
                    raise Exception(f"{validator_name} - Built-in validator specified not known") from KeyError
    ...
    def rule_splitter(data):
        ...
        if not len(rules) > 1:
            return validator, []
        args = tuple((rules[1],) if validator == "regex" else rules[1].split(","))
...
class Validators:
    ...
    def max(request_data, *validator_args):
        error_msg = f"This field must not be greater than {validator_args[0]}"
    ...
    def min(request_data, *validator_args):
        error_msg = f"This field must not be less than {validator_args[0]}"
        if isinstance(request_data, int):
            if request_data < int(validator_args[0]):
    ...
    def list(request_data, *validator_args):
        ...
        if validator_args:
            if validator_args[0] and len(request_data) != validator_args[0]:
                error_msg += f" with length of {validator_args[0]}"
                return {"status": False, "message": error_msg}
    ...
    def date(request_data, *args):
        error_msg = f"This field must be a date that match this format {args[0]}"
        try:
        ...
        if len(args) == 2:
            ...
            if date_value_1 != date_value_2:
                error_msg += f"and value must be {args[1]}"
                return {"status": False, "message": error_msg}
    ...
    def after(request_data, *args):
        if len(args) != 2:
            raise Exception("ArgumentError - Usage should be date_after:<format>,<value>")

        error_msg = f"This field must be after this date {args[0]}"

        try:
            date_value_1 = datetime.strptime(request_data, args[0])
        except ValueError:
            return {"status": False, "message": f"This field must be a date that match this format {args[0]}"}
    ...
    def after_or_equal(request_data, *args):
        if len(args) != 2:
            raise Exception("ArgumentError Usage should be date_after_or_equal:<format>,<value>")

        error_msg = f"This field must be after or equal to this date {args[0]}"
    ...
    def before(request_data, *args):
        if len(args) != 2:
            raise Exception("ArgumentError - Usage should be date_before:<format>,<value>")

        error_msg = f"This field must be before this date {args[0]}"
        ...
        try:
            ...
            return {
                "status": False,
                "message": f"This field must be a date that match this format {args[0]}",
            }
    ...
    def before_or_equal(request_data, *args):
        if len(args) != 2:
            raise Exception("ArgumentError - Usage should be date_before_or_equal:<format>,<value>")

        error_msg = f"This field must be before or equal to this date {args[0]}"
